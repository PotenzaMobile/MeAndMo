...
import com.cipl.meandmo.adapter.CartAdapter;
...
import com.cipl.meandmo.model.Customer;
...
public class CartActivity extends BaseActivity implements OnItemClickListner, OnResponseListner {
    ...
    public void setTotalCount() {
        ...
        float amount = 0;
        for (int i = 0; i < cartAdapter.getList().size(); i++) {
            listCspPrice = new Gson().fromJson(cartAdapter.getList().get(i).getCspPrice(), new TypeToken<ArrayList<CspPrice>>() {
            }.getType());
            if (cartAdapter.getList().get(i).getCategoryList().taxPrice != null && cartAdapter.getList().get(i).getCategoryList().taxPrice.length() > 0 && !cartAdapter.getList().get(i).getCategoryList().taxPrice.equals("0.0")) {
                ...
                try {
                    if (listCspPrice.isEmpty()) {
                        ...
                        for (int j = 0; j < listCspPrice.size(); j++) {
                            if (Integer.parseInt(listCspPrice.get(j).minQty) <= cartAdapter.getList().get(i).getQuantity()) {
                                amount = amount + (Float.parseFloat(getPrice(listCspPrice.get(j).price)) * cartAdapter.getList().get(i).getQuantity());
                            } else {
                                amount = amount + (Float.parseFloat(getPrice(cartAdapter.getList().get(i).getCategoryList().price)) * cartAdapter.getList().get(i).getQuantity());
                            }
                        }
